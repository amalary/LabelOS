...
from labelos_api.logging import (
    JsonFormatter,
    configure_logging,
    correlation_headers,
    get_request_id,
...
def test_json_formatter_emits_cloud_logging_fields() -> None:
    ...
    assert payload["message"] == "database write failed"
    assert payload["service"] == "labelos-api"
    ...
    assert payload["metadata"] == {"refresh_token": "[REDACTED]", "safe": "value"}
...
def test_request_logs_include_correlation_context(
    ...
    app = create_app()
    logger = logging.getLogger("labelos_api.test")
