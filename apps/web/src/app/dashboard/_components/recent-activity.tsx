...
const toneClassNames: Record<ActivityEventTone, string> = {
  organization: "bg-sky-300 shadow-[0_0_12px_rgba(125,211,252,0.22)]",
  team: "bg-emerald-300 shadow-[0_0_12px_rgba(110,231,183,0.2)]",
  artist: "bg-cyan-300 shadow-[0_0_12px_rgba(34,211,238,0.22)]",
  release: "bg-violet-300 shadow-[0_0_12px_rgba(196,181,253,0.2)]",
  campaign: "bg-amber-300 shadow-[0_0_12px_rgba(252,211,77,0.18)]",
  approval: "bg-rose-300 shadow-[0_0_12px_rgba(253,164,175,0.18)]",
  agent: "bg-indigo-300 shadow-[0_0_12px_rgba(165,180,252,0.2)]",
  default: "bg-slate-300 shadow-[0_0_12px_rgba(203,213,225,0.16)]",
};

function RecentActivityLoading() {
  return (
    <ol className="flex flex-col gap-3.5" aria-label="Recent activity loading">
      ...
      <div className="flex flex-col gap-4">
        ...
        {!activity.loading && !activity.error && items.length === 0 ? (
          <div className="rounded-lg border border-slate-700/80 bg-slate-950/30 px-3 py-4">
            ...
            <Link
              className="mt-3 inline-flex rounded-sm text-sm font-semibold text-cyan-100 hover:text-cyan-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950"
          ...
          <ol className="flex flex-col gap-3.5">
            {items.map((item) => (
              <li className="grid grid-cols-[auto_1fr] gap-3" key={item.id}>
                <span
                  className={`mt-1.5 h-2 w-2 rounded-full ${toneClassNames[item.tone]}`}
                  aria-hidden="true"
                />
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <p className="text-sm font-semibold text-slate-100">{item.title}</p>
                    <span className="text-xs font-medium text-slate-400">{item.rawType}</span>
                  </div>
