...
export function GET(request: Request, context: { params: Promise<{ workspaceId: string }> }) {
  ...
  return context.params.then(({ workspaceId }) =>
    proxyWorkspaceRequest(
      `/api/v1/workspaces/${workspaceId}/analytics/metric-definitions${query}`,
      {
        headers: {
...
export async function POST(
  ...
  return proxyWorkspaceRequest(`/api/v1/workspaces/${workspaceId}/analytics/metric-definitions`, {
    method: "POST",
